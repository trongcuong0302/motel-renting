...
export class UserListComponent extends BaseComponent implements OnInit {
  ...
  refreshCheckedStatus(): void {
    this.checked = this.userList.every((item :any) => this.setOfCheckedId.has(item.id));
    this.indeterminate = this.userList.some((item :any) => this.setOfCheckedId.has(item.id)) && !this.checked;
  }

  onAllChecked(value: boolean): void {
    this.userList.forEach((item :any) => this.updateCheckedSet(item.id, value));
